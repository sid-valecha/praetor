...
- `--github-pr N --respond-to-review --max-cycles 3` runs the first conservative focused PR response slice: read loop state, exit cleanly when clean, and create deterministic repair proposals or tasks only when the PR needs repair and task writing is explicitly authorized.
- Add `--run-repairs --task-verify "..."` to let the focused PR response slice drain only those repair tasks through the local executor, verify, and reviewer gates.
...
| `praetor maintain` | `--once`, `--github`, `--github-pr`, `--github-issue`, `--propose-tasks`, `--respond-to-review`, `--max-cycles`, `--write-tasks`, `--task-verify`, `--run-repairs`, `--adapter`, `--model`, `--effort`, `--reviewer-*`, `--json` | Maintainer intake for local state triage and optional read-only GitHub issue/PR/CI intake. `--propose-tasks` converts findings into deterministic repair proposals; `--write-tasks` materializes generated proposals as deterministic `.praetor/tasks/*.md` files. `--github-pr N --respond-to-review --run-repairs` drains only the generated repair tasks through local executor, verify, and reviewer gates. Conservatively, the shipped command does not perform autonomous GitHub mutation, branch push, review-request, CI rerun, or merge actions. |
