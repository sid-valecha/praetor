...
| `praetor maintain` | `--once`, `--github`, `--github-pr`, `--github-issue`, `--propose-tasks`, `--json` | Experimental read-only maintainer pass for Praetor task/run-state triage and optional GitHub issue/PR/CI intake. `--propose-tasks` converts applicable GitHub findings into task-shaped proposals without writing task files; combine it with `--github`, `--github-pr N`, or `--github-issue N` to target GitHub feedback. |
