...
| `praetor maintain` | `--once`, `--github`, `--github-pr`, `--github-issue`, `--propose-tasks`, `--respond-to-review`, `--max-cycles`, `--write-tasks`, `--task-verify`, `--json` | Maintainer intake for local state triage and optional read-only GitHub issue/PR/CI intake. `--propose-tasks` converts findings into deterministic repair proposals; `--write-tasks` materializes generated proposals as deterministic `.praetor/tasks/*.md` files. `--github-pr N --respond-to-review` runs the first bounded PR review-response slice and stops at report/proposal/task creation. Conservatively, the shipped command does not perform autonomous GitHub mutation, branch push, review-request, CI rerun, or merge actions. |
